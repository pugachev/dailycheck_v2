...
class QueryDaily extends connect
{
    ...
    public function save()
    {
        ...
        $tgttimezone = $this->daily->getTimezone();
        $tgtfilename = $this->daily->getPictheme();;
        ...
        if ($this->daily->getId()) {
            ...
            SET tgtdate=:tgtdate, tgtcategory=:tgtcategory, tgtitem=:tgtitem, tgtmoney=:tgtmoney, tgtcalory=:tgtcalory,tgttimezone=:tgttimezone,tgtpicname=:tgtfilename,updated_at=NOW() WHERE id=:id");
            $stmt->bindParam(':id', $id, PDO::PARAM_INT);
         } else {
            // IDがなければ新規作成
            $stmt = $this->dbh->prepare("INSERT INTO records (tgtdate, tgtcategory, tgtitem, tgtmoney, tgtcalory,tgttimezone,tgtpicname,created_at, updated_at)
                        VALUES (:tgtdate, :tgtcategory, :tgtitem, :tgtmoney,:tgtcalory, :tgttimezone,:tgtfilename,NOW(), NOW())");
        }
        ...
        $stmt->bindParam(':tgttimezone', $tgttimezone, PDO::PARAM_INT);
        $stmt->bindParam(':tgtfilename', $tgtfilename, PDO::PARAM_STR);
    ...
    public function getPicdata($tgtdate,$timezone){
        $stmt = $this->dbh->prepare("SELECT  tgtpicname FROM records WHERE tgtdate=:tgtdate and tgttimezone=:tgttimezone");
        $stmt->bindParam(':tgtdate', $tgtdate, PDO::PARAM_STR);
        $stmt->bindParam(':tgttimezone', $timezone, PDO::PARAM_STR);
        $stmt->execute();
        // $stmt->debugDumpParams();

        $picdata = $this->getPicName($stmt->fetchAll(PDO::FETCH_ASSOC));
        return $picdata;
    }
    ...
    private function getPicName($results)
    {
        $picdata = "";
        if(empty($results)){
